...
namespace DomainDrivenDesign.CrossCutting.Identity.Configuration
{
        ...
        IUserTwoFactorStore<TUser, string>, IDisposable where TUser : IdentityUser
    {
        ...
        public virtual Task<TUser> FindAsync(UserLoginInfo login)
        {
            ...
            var query = from u in Session.Query<TUser>()
                        from l in u.Logins
        ...
        public virtual Task AddToRoleAsync(TUser user, string role)
        {
            ...
            IdentityRole identityRole = Queryable.SingleOrDefault(Session.Query<IdentityRole>(), (r => r.Name.ToUpper() == role.ToUpper()));
        ...
        private Task<TUser> GetUserAggregateAsync(Expression<Func<TUser, bool>> filter)
        {
            return Task.Run(() =>
            {
                ...
                var query = Session.Query<TUser>().Where(filter);
